...
| QuickJS Native Module                             | Extension | Host owns                  | ⚠️ Unsafe    | Initialization flag `QjsInitializeFlags::AddonMode` is required <br/> ⚠️ Pay attention to compilation flags and ABI issues <br/> ⚠️ **Unsafe**: native modules have no cleanup hook that runs while the `JSContext` is still usable — addon-held JSValues (stored callbacks, registered classes, `Global<T>`-style references) are never released until `JS_FreeRuntime`, and leak entirely if the addon still references them from C++ (debug builds assert, release builds leak silently). See [quickjs-ng/quickjs#1703](https://github.com/quickjs-ng/quickjs/issues/1703)                                                                                                                                                                                                                                                                                                                                     |
| QuickJS Native Module (Host using jspp)           | Extension | Host's jspp owns           | ⚠️ Unsafe    | Initialization flag `QjsInitializeFlags::AddonMode` is required <br/> ⚠️ Pay attention to compilation flags and ABI issues <br/> ⚠️ **Unsafe**: native modules have no cleanup hook that runs while the `JSContext` is still usable — addon-held JSValues (stored callbacks, registered classes, `Global<T>`-style references) are never released until `JS_FreeRuntime`, and leak entirely if the addon still references them from C++ (debug builds assert, release builds leak silently). See [quickjs-ng/quickjs#1703](https://github.com/quickjs-ng/quickjs/issues/1703)                                                                                                                                                                                                                                                                                                                                     |
| QuickJS Native Module (Host using other bindings) | Extension | Host owns                  | ⚠️ Unsafe    | Initialization flag `QjsInitializeFlags::AddonMode` is required <br/> ⚠️ Pay attention to compilation flags and ABI issues <br/> ⚠️ **Unsafe**: native modules have no cleanup hook that runs while the `JSContext` is still usable — addon-held JSValues (stored callbacks, registered classes, `Global<T>`-style references) are never released until `JS_FreeRuntime`, and leak entirely if the addon still references them from C++ (debug builds assert, release builds leak silently). See [quickjs-ng/quickjs#1703](https://github.com/quickjs-ng/quickjs/issues/1703) <br/> Compatibility has been tested with ScriptX; ⚠️ if the host's bound interfaces obtain the engine via their binding library's `EngineScope` (TLS scope chain) without null checks, they will crash with a null-pointer access on call paths where no `EngineScope` has been pushed; the same applies to other binding libraries |
...
> **Note**: QuickJS **Addon mode is unsafe** for stateful bindings. Native C modules (`JS_NewCModule`) have no cleanup hook that runs while the `JSContext` is still usable — module teardown is deferred to `JS_FreeRuntime`'s GC, and never runs at all if the addon still holds any JSValue from C++ (a stored callback, a registered class prototype, a `Global<T>`): debug builds end in `assert(list_empty(&rt->gc_obj_list))`, release builds silently leak every bound JSValue. Under the standard module spec only pure (stateless) computational modules can be authored safely; stateful addons require a custom host that drives cleanup before `JS_FreeContext`. See [quickjs-ng/quickjs#1703](https://github.com/quickjs-ng/quickjs/issues/1703).
